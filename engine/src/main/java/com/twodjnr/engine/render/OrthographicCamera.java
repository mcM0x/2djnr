...
public class OrthographicCamera {
    ...
    public void update() {
        // Projection: ortho centered at (x,y) with zoom
        float hw = viewportWidth / (2.0f * zoom);
        float hh = viewportHeight / (2.0f * zoom);
        ortho(projection, x - hw, x + hw, y + hh, y - hh, -1, 1);

        // View: identity (we bake position into projection)
        identity(view);

        // Combined = projection * view (view is identity, so combined = projection)
